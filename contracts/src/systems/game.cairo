...
use ryo_pvp::models::hustler::TwoHustlerTrait;
use ryo_pvp::models::hustler::HustlerTrait;
use dojo::world::{IWorldDispatcher, IWorldDispatcherTrait};
use starknet::{ContractAddress, get_caller_address};
use ryo_pvp::{models::{game::{TwoPlayerGame, Move, TwoMoves}, hustler::{Hustler, TwoHustlers, HustlerState, TwoHustlersState}, utils::{AB, TwoTrait}, commits::{CommitHash, HashTrait, CommitHashTrait, TwoCommits, TwoCommitsTrait}, map::Vec2, drugs::Drugs}, mechanics::run_round};
...
impl GameImpl of GameTrait {
    ...
    fn get_hustlers(self: Game) -> TwoHustlers{
        TwoHustlers{
            a: get!(self.world, (self.game_id, self.hustler_a), Hustler),
            b: get!(self.world, (self.game_id, self.hustler_b), Hustler),
        }
    }
    fn get_states(self: Game)-> TwoHustlersState{
        TwoHustlersState{
            a: get!(self.world, (self.game_id, self.hustler_a), HustlerState),
            b: get!(self.world, (self.game_id, self.hustler_b), HustlerState),
        }
    }
    ...
    fn get_move(self: Game, huster_id: u128)-> Move{
        get!(self.world, (self.game_id, hustler_id), Move)

    }
    fn get_moves(self: Game)->TwoMoves{
        TwoMoves{
            a: get!(self.world, (self.game_id, self.hustler_a), Move),
            b: get!(self.world, (self.game_id, self.hustler_b), Move),
        }
    }
    ...
    fn get_commits(self: Game) -> TwoCommits{
        let commits: TwoCommits = TwoTrait::create(get!(self.world, (self.game_id, self.hustler_a)),get!(self.world, (self.game_id, self.hustler_b)));
    ...
    fn reveal_hustler(self: Game, hustler: Hustler, salt: felt252) {
        ...
        assert(hustler.get_hash(salt) == commits.get(hustler.hustler_id), 'Hash dose not match');
    ...
    fn reveal_move(self: Game, move: Move, salt: felt252) {
        ...
        assert(move.get_hash(salt) == commits.get(move.hustler_id), 'Hash dose not match');
        
        let mut other_move = self.get_move(self.get_other_hustler_id(move.hustler_id));
        if other_move.revealed{
            moves.set(move);
            self.reset_hashes();
            let (state_a, state_b) = run_round(self.get_hustlers(), self.get_states(),  moves); 
            other_move.revealed = false;
            set!(self.world, (other_move, state_a, state_b));
            self.reset_hashes();
        } else {
            set!(self.world, (move, ));
        }
